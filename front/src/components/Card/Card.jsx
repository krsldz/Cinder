...
function CardSolo({ id }) {
  ...
  const movieInfo = (id) => {
    fetch(
      `https://api.kinopoisk.cloud/movies/1143242/token/76cf037e01db02ca1036a7cf5e350b1c`
    )
      .then((res) => res.json())
  ...
  return (
    <div className='card'>
      ...
      {trailerPreview && <Trailer trailer={infoAboutMovie.trailer} />}
    </div>
  );
}

export default React.memo(CardSolo);
